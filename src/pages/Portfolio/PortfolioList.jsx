...
export default function PortfolioList() {
  ...
  const actions = (item) => (
    <div className="flex space-x-2">
      <Link to={`/portfolio/edit/${item.id}`} className="text-blue-600 hover:text-blue-900">Edit</Link>
      <button
  ...
  if (isError) return <div className="p-6 text-red-500">Error: {error?.message || 'Failed to load portfolio'}</div>;

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Portfolio</h1>
        <Link to="/portfolio/create" className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
